...
namespace PVBrand
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config = new Menu("PVBrand", "PVBrand", true);
            ...
            Config.SubMenu("Combo").AddItem(new MenuItem("useR.count.target", "Use R >= Target").SetValue(new Slider(2, 1, 5)));
        ...
        private static void OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            if (gapcloser.Sender.HasBuff("brandablaze"))
            {
                if (_Q.IsReady())
                {
            ...
            else
            {
                if (_Q.IsReady())
                {
                    if (_E.IsReady())
                    {
                        CastE(gapcloser.Sender, Config.SubMenu("Combo").Item("useE").GetValue<bool>());
                    }
                }
                if (_W.IsReady())
                {
                    CastW(gapcloser.Sender, Config.SubMenu("Combo").Item("useW").GetValue<bool>());
        ...
        private static void CastQ(Obj_AI_Hero target, bool CheckConfig)
        {
        ...
        private static void CastW(Obj_AI_Hero target, bool CheckConfig)
        {
            if (_W.IsReady() && target.IsValidTarget(_W.Range - 50))
            {
                if (CheckConfig)
                {
        ...
        private static void CastE(Obj_AI_Hero target, bool CheckConfig)
        {
        ...
        private static void CastR(Obj_AI_Hero target, bool CheckConfig)
        {
            if (_R.IsReady() && target.IsValidTarget(_R.Range))
            {
                if (CheckConfig)
                {
                    if (Player.CountEnemiesInRange(1000) >= Config.SubMenu("Combo").Item("useR.count.target").GetValue<Slider>().Value)
        ...
        {
            if (CheckMana(_E))
                CastE(target, Config.SubMenu("Harass").Item("useE").GetValue<bool>());
            if (CheckMana(_Q))
                CastQ(target, Config.SubMenu("Harass").Item("useQ").GetValue<bool>());
            if (CheckMana(_W))
                CastW(target, Config.SubMenu("Harass").Item("useW").GetValue<bool>());
        }
        ...
        private static void Combo(Obj_AI_Hero target)
        {
            ...
            if (_orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
            {
                if (_W.IsReady())
                {
                    CastW(target, Config.SubMenu("Combo").Item("useW").GetValue<bool>());
                }
                if (_E.IsReady())
                {
                    CastE(target, Config.SubMenu("Combo").Item("useE").GetValue<bool>());
                }
                if (_R.IsReady())
                {
                    CastR(target, Config.SubMenu("Combo").Item("useR").GetValue<bool>());
                }
                if (target.HasBuff("brandablaze"))
                {
                    CastQ(target, Config.SubMenu("Combo").Item("useQ").GetValue<bool>());
